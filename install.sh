...
# "contracts HQ", not a monorepo: each tool lives in its own repo. So this
...
# `r-<tool>` wrapper scripts + shell aliases.
#
# Usage:
...
# The Python tool(s): "repo_name:entrypoint". toolbox-bridge ships bridge.py.
PY_TOOLS=(
  "toolbox-bridge:bridge.py"
)
...
check_prereqs() {
  ...
  # Python path is best-effort: warn, don't fail (the Rust suite still installs).
  if command -v pipx >/dev/null 2>&1; then
    ok "pipx $(pipx --version 2>/dev/null || echo present)"
  elif command -v python3 >/dev/null 2>&1; then
    warn "pipx not found; will fall back to a venv for Python tools (python3 present)"
  else
    warn "neither pipx nor python3 found — Python tools (toolbox-bridge) will be skipped"
  fi
...
install_py_tool() {
  local repo="$1" entry="$2" dir="$3"
  local target="${BIN_DIR}/${repo}"

  step "Installing $repo (python)"

  # Prefer pipx if there's a packageable project; else install a thin launcher
  # that runs the script from its repo via a dedicated venv.
  if command -v pipx >/dev/null 2>&1 && { [ -f "$dir/pyproject.toml" ] || [ -f "$dir/setup.py" ]; }; then
    local pipx_force=()
    [ "$FORCE" -eq 1 ] && pipx_force=(--force)
    if run pipx install "${pipx_force[@]}" "$dir"; then
      ok "$repo installed via pipx"
      return 0
    fi
    warn "$repo: pipx install failed — falling back to venv launcher"
  fi

  if ! command -v python3 >/dev/null 2>&1; then
    warn "$repo: python3 not available — skipping"
    return 1
  fi

  local venv="${dir}/.venv-suite"
  if [ ! -d "$venv" ] || [ "$FORCE" -eq 1 ]; then
    run python3 -m venv "$venv"
    # Install deps if a requirements file exists; best-effort (don't abort the
    # suite if pip fails — the launcher may still work for a stdlib-only script).
    if [ -f "$dir/requirements.txt" ]; then
      run "$venv/bin/pip" install -q -r "$dir/requirements.txt" || warn "$repo: pip install had errors (continuing)"
    fi
  fi

  # A launcher on PATH that calls the repo's entrypoint with the venv python.
  if [ "$DRY_RUN" -eq 1 ]; then
    printf '  %s(dry-run)%s write launcher %s -> %s/%s\n' "$C_DIM" "$C_RST" "$target" "$repo" "$entry" >&2
  else
    mkdir -p "$BIN_DIR"
    cat > "$target" <<LAUNCH
#!/usr/bin/env bash
# Auto-generated by linux-ops-suite/install.sh — launcher for ${repo}.
exec "${venv}/bin/python" "${dir}/${entry}" "\$@"
LAUNCH
    chmod +x "$target"
  fi
  ok "$repo → ${target} (venv launcher)"
}
...
install_wrappers_and_aliases() {
  ...
  for pair in "${PY_TOOLS[@]}";  do all+=("${pair%%:*}"); done
...
main() {
  ...
  for pair in "${PY_TOOLS[@]}"; do
    local repo="${pair%%:*}" entry="${pair##*:}"
    selected "$repo" || continue
    if dir="$(ensure_repo "$repo")"; then
      install_py_tool "$repo" "$entry" "$dir" || true
    fi
  done
