...
|---|---|---|
| `linux-ops-suite` | `thomas-tui`, `suite-ui`, `toolbox-bridge`, `linux-ops-install`, `rex-check` | Umbrella: the general TUI toolkit (`thomas-tui`) + the suite chrome layered on it (`suite-ui`) + the Rust bridge (`toolbox-bridge`) + the prebuilt-release installer (`linux-ops-install`) + the suite-repo health summary (`rex-check`). Owns `docs/INTEGRATION_MAP.md` and the `contracts/` JSON schemas. |
