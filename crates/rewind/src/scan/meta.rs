//! Turning a path into a [`CaptureEntry`]'s metadata, dependency-free. Uses
...
//! the Unix mode/uid/gid bits. Content for files is streamed through SHA-256 in
//! [`hash_file`] so a large captured file never loads wholesale into memory.
//!
//! Lifted from tripwire's `scan/meta.rs`; only the [`EntryKind`] source differs.
...
/// The non-content metadata rewind records for any path: its kind plus the
...
impl Meta {
    /// Build from a [`Metadata`]. The caller decides which metadata to pass
    /// (lstat vs stat) via the symlink-follow policy; here we just describe it.
...
/// (`0644`), masking off the file-type bits — those are carried by `kind`.
pub fn format_mode(raw_mode: u32) -> String {
    format!("{:04o}", raw_mode & 0o7777)
}

/// Format a file's mtime as an RFC3339 UTC string. Returns `None` if the time is
...
/// string. Self-contained civil-date calculation (Howard Hinnant's
/// `days_from_civil` inverse) so there's no date dependency here. (The capture
/// timestamp itself uses chrono in [`crate::capture`]; this is the per-file
/// mtime, kept dependency-free to match the lifted tripwire code.)
...
/// Stream a file through SHA-256, returning its lowercase hex digest. `Err`
/// means the file exists but couldn't be read — the caller turns that into an
/// `unreadable` entry, never a hard failure.
...
mod tests {
    ...
    fn meta_classifies_file_and_dir() {
        ...
        assert_eq!(Meta::from_metadata(&dmd).kind, EntryKind::Dir);
    }
